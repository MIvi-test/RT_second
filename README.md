...
Перед запуском необходимо настроить пути  исходного файлов: SOURCE, SCORE_PATH, EVAL_PATH. Опционально: BD_PATH, HF_TOKEN и HF_HOME.

| Переменная   | По умолчанию                                   | Описание                                |
| ------------ | ---------------------------------------------- | --------------------------------------- |
| `SCORE_PATH` | `.\dataset_case3_v1.0_fix\score.py`            | Путь к `score.py`                       |
| `SOURCE`     | `${USERPROFILE}\Downloads\dataset}`            | Путь к исходному коду для индексации    |
| `EVAL_PATH`  | `.\dataset_case3_v1.0_fix\eval_questions.json` | Путь к `eval_questions.json`            |
| `BD_PATH`    | `.\storage`                                    | Каталог с индексами (ChromaDB, BM25)    |
| `HF_TOKEN`   | `отсутствует`                                  | для быстрого скачивания моделей         |
...
- USE_OLLAMA=false
- STORAGE_DIR=/storage
- SOURCE_PATH=/dataset_case3_v1.0_fix
```
...
Файл разбирается через `ast.parse`, после чего обходится только верхний уровень `tree.body`:

1. **Функции верхнего уровня** (`FunctionDef` / `AsyncFunctionDef`) — каждая становится отдельным чанком.
2. **Классы** (`ClassDef`) индексируются **дважды**:
    - один чанк — весь класс целиком (для запросов об архитектуре/интерфейсе);
    - по чанку на каждый метод внутри класса (для точечных запросов по логике).
3. Для каждого чанка формируется:
    - `id` вида `rel_path:name:lineno` (уникальный ключ);
    - `document` — текстовое представление с мета-заголовком и исходным кодом, извлечённым через `get_node_source` по `lineno` / `end_lineno`;
    - `metadata` — структурированные поля (`file_path`, `name`, `type`, `start_line`, `end_line`) для фильтрации в ChromaDB.

#### Почему такая реализация оптимальна

Нарезка по AST-границам сохраняет семантическую целостность сущностей, в отличие от слепой токенизации по символам, которая рвёт функции и классы посередине. Двойная индексация класса (целиком + методы) даёт гибкость поиска: общие запросы попадают в описание всего класса, а точечные — сразу в нужный метод без шума. Встроенный в документ мета-заголовок обогащает эмбеддинг контекстом (путь, тип, имя), что существенно повышает качество retrieval для семантических моделей.

### Индексация (`index_java.py`)

- **Tree-sitter парсинг** — вместо стандартного `ast` используется `tree-sitter` + `tree-sitter-java`, что даёт устойчивый парсинг даже при синтаксических особенностях разных версий JDK. Обходятся узлы типов (`class_declaration`, `interface_declaration`, `enum_declaration`, `annotation_type_declaration`, `record_declaration`) и их вложенные типы рекурсивно. Дефектные файлы пропускаются с предупреждением.
- **Metadata Injection** — перед векторизацией в текст чанка добавляется заголовок с путём к файлу, типом и именем объекта: `File path: ...\nObject type: ...\nObject name: ...\nCode: ...`. Это позволяет модели учитывать контекст расположения кода.
- **Токенизатор `tokenize_code`** — разбивает `camelCase` на отдельные токены (`createUser` → `create user`), что критически повышает точность BM25 при поиске по именам функций и классов (в Java это основной стиль именования).
- **ChromaDB** — используется та же коллекция `code_chunks` с метрикой косинусного расстояния (`hnsw:space: cosine`), запись батчами по 200 элементов. При повторном запуске коллекция не пересоздаётся — используется `get_or_create_collection`, после чего выборочно удаляются только старые записи Java-файлов (`where={"file_path": {"$like": "%.java"}}`), чтобы не затронуть записи других языков (например, Python).
- **BM25-индекс** — строится наряду с ChromaDB и сохраняется на диск в отдельные файлы (`bm25_index_java.pkl` + `bm25_meta_java.json`) в `STORAGE_DIR`, чтобы не конфликтовать с Python-индексом.
#### Реализация чанкования

Файл разбирается через `_parser.parse(src_bytes)`, после чего обходится `tree.root_node` (тип `compilation_unit`):

1. **Верхнеуровневые типы** (`class` / `interface` / `enum` / `annotation` / `record`) — каждый индексируется целиком как отдельный чанк.
2. **Вложенные типы** обрабатываются рекурсивно через `_walk_type_node`, имя формируется как `OuterClass.InnerClass`.
3. **Методы и конструкторы** внутри каждого типа индексируются отдельными чанками с именем вида `ClassName.methodName`.
4. Для каждого чанка формируется:
    - `id` вида `rel_path:full_name:start_line` (уникальный ключ);
    - `document` — текстовое представление с мета-заголовком и исходным кодом узла, извлечённым по `start_byte` / `end_byte`;
    - `metadata` — структурированные поля (`file_path`, `name`, `type`, `start_line`, `end_line`) для фильтрации в ChromaDB.
### Поиск (`search.py`)
...
| `hybrid_search`   | ChromaDB (50%) + BM25 (50%) → топ-25 кандидатов → (опционально) реранкер → топ-5 |
